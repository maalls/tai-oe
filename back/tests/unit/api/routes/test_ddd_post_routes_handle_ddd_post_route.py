...
def test_handle_ddd_post_route_returns_unhandled_for_retired_legacy_paths():
    handled, payload, status = handle_ddd_post_route("/api/opportunity/advance", {}, handlers=object())
...
def test_is_ddd_post_route_is_false_for_any_path_with_empty_route_map():
    assert is_ddd_post_route("/api/opportunity/advance") is False
    assert is_ddd_post_route("/api/rfp/submit") is False
