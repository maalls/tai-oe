...
def test_dispatch_post_business_routes_delegates_invoice_router(monkeypatch):
    calls = []

    def _invoice_router(handler, method, parsed, qs, request_handlers):
        calls.append((handler, method, parsed.path, qs, request_handlers))
        return True

    monkeypatch.setattr("src.api.routes.dispatchers.server_post_dispatch.dispatch_invoice_routes", _invoice_router)

    handler = _HandlerStub()
    parsed = SimpleNamespace(path="/api/quote/doc-8/invoice")

    handled = dispatch_post_business_routes(handler, parsed)

    assert handled is True
    assert calls == [(handler, "POST", "/api/quote/doc-8/invoice", {}, handler.request_handlers)]
