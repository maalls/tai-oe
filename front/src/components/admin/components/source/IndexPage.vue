...
<template>
   <div class="min-h-screen">
      <AdminNavHeader />
      <!-- Source Selection -->
      <div class="m-2 hidden sm:flex items-center justify-between gap-6">
         <select
            id="sourceSelect"
         ...
         <div class="ml-auto flex items-center gap-2">
            <button
               type="button"
               class="px-3 py-2 bg-emerald-600 text-white rounded-md text-sm hover:bg-emerald-700 transition"
            ...
            <a
               ...
               href="#"
               target="_blank"
               class="px-3 py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 transition"
            >
               Download XLS
            </a>
         </div>
      ...
      <div
         id="tabs"
         class="scroll-container mt-4 flex gap-2 items-center mb-4 border-b border-gray-300 pl-2 overflow-x-auto no-scrollbar"
      ></div>
...
const api = {
   ...
   raw: (source) => ORIGIN + `/api/csv/source?source=${encodeURIComponent(source)}`,
};
...
function updateDownloadLink(file) {
   ...
   if (link) link.href = api.raw(currentSourceFile);
}
...
function setStatus(text) {
   // Status bar removed - this function is kept for compatibility but does nothing
}
...
async function performSearch() {
   ...
   try {
      ...
      const file = getCurrentSheet();
      const fullCsvLimit = 99999; // Search entire CSV
      ...
      document.getElementById('pageInfo').textContent =
         `Search Results | ${count} of ${total} matches`;
...
function exportTableAsCSV(filename) {
   // Download the actual CSV file from storage
   const source = currentSourceFile;
   const sheet = getCurrentSheet();

   if (!source || !sheet) {
      console.error('Missing source or sheet');
      setStatus('Error: Missing source or sheet');
      return;
   }
...
function setupEventListeners() {
   ...
   document.getElementById('downloadSearchBtn')?.addEventListener('click', () => {
      try {
         console.log('Download button clicked');
         if (currentSearchField && currentSearchText) {
            // If search is active, export filtered results as CSV
            exportTableAsCSV(`${currentSourceFile}_${currentSearchField}_${currentSearchText}.csv`);
         } else {
            // Otherwise, export current view as CSV
            exportTableAsCSV(`${currentSourceFile}.csv`);
         }
      } catch (e) {
