   ...
   <div class="list-page-stack">
      ...
      <div v-else class="list-card">
            ...
            <table class="list-table">
               <thead>
                  <tr>
                     ...
                     <th class="list-table-right">
                        {{ t('products.brand.columns.netPrices') }}
                     </th>
                     <th class="list-table-right">
                  ...
                  >
                     ...
                     <td class="list-table-right" @click.stop>
                        <router-link
                           :to="{
                              path: '/vendors/family',
                              query: { brand_id: brand.id, tab: 'all' },
                           }"
                     ...
                     <td class="list-table-right" @click.stop>
                        <router-link
                           :to="{
                              path: '/vendors/family',
                              query: { brand_id: brand.id, tab: 'net_price' },
                           }"
                           class="list-table-link"
                        >
                           {{ familiesWithNetPriceByBrand[brand.id]?.length || 0 }}
                        </router-link>
                     </td>
                     <td class="list-table-muted list-table-right">
                        {{
         ...
         <div v-if="!brands.length && !isLoading" class="p-6 text-center text-gray-500">
            {{ emptyMessage || t('products.brand.noBrands') }}
         </div>
      </div>
   </div>
</template>
...
import type { Brand, Family } from '../useBrandFamilyData';
...
const familiesWithNetPriceByBrand = computed(() => {
   return props.families.reduce(
      (acc, family) => {
         const isNetPriceType = String(family.type || '').toLowerCase() === 'net_price';
         const hasNetPrice = Number.isFinite(Number(family.net_price));

         if (isNetPriceType && hasNetPrice) {
            if (!acc[family.brand_id]) acc[family.brand_id] = [];
            acc[family.brand_id]!.push(family);
         }
         return acc;
