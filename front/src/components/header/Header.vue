<template>
   <header class="bg-black shadow-lg sticky top-0 z-50">
      <nav class="flex gap-2 px-6 py-4 items-center">
         ...
         <!-- Si l'utilisateur n'est pas "anon", on affiche le menu -->
         <template v-if="effectiveUserRole !== 'anon'">
            <router-link
...
// Si userRole est null ou vide, on considère que l'utilisateur est "anon"
const effectiveUserRole = computed(() => userRole.value || 'anon');
...
const visiblePages = computed(() =>
   pages.filter((page) => page.role !== 'admin' || effectiveUserRole.value === 'admin')
);
