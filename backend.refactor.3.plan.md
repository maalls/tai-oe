...
8. fait: migrer `PipelineStageInvoiced.vue` hors `supabase.from('document'|'opportunity')`

- fait: ajout endpoint backend `PUT /api/document/{id}/status`
- fait: extension client frontend `front/src/api/document.ts` (`updateDocumentStatus`)
- fait: `front/src/components/opportunity/components/pipeline/components/PipelineStageInvoiced/PipelineStageInvoiced.vue` migre vers APIs backend `document`/`opportunity`
- fait: tests unitaires backend/frontend ajoutes pour la nouvelle route/client
- fait: guardrail supabase-direct valide, baseline reduite a `20` fichiers

9. fait: migrer `PipelineStagePaid.vue` hors `supabase.from('document'|'opportunity')`

- fait: `front/src/components/opportunity/components/pipeline/components/PipelineStagePaid/PipelineStagePaid.vue` migre vers APIs backend `document`/`opportunity`
- fait: tests unitaires frontend executes sur les clients API utilises
- fait: guardrail supabase-direct valide, baseline reduite a `19` fichiers

10. fait: migrer `StageManager.vue` hors `supabase.from('opportunity'|'opportunity_state_transition')`

- fait: ajout endpoints backend `GET /api/opportunity/{id}/stage-history` et `PUT /api/opportunity/{id}/stage-state`
- fait: extension client frontend `front/src/api/opportunity.ts` (`getOpportunityStageHistory`, `updateOpportunityStageState`)
- fait: `front/src/components/opportunity/components/pipeline/components/StageManager.vue` migre vers APIs backend pour stage/status + historique
- fait: alignement `PipelineStagePaid.vue` sur `updateOpportunityStageState` pour conserver `status=WON` en fermeture
- fait: tests unitaires backend/frontend ajoutes pour les nouveaux endpoints/helpers
- fait: guardrail supabase-direct valide, baseline reduite a `18` fichiers

11. fait: migrer `PipelineStageRfp.vue` hors `supabase.from('document')`

- fait: `front/src/components/opportunity/components/pipeline/components/PipelineStageRfp/PipelineStageRfp.vue` migre vers API backend `document`
- fait: tests unitaires frontend executes sur le client API utilise
- fait: guardrail supabase-direct valide, baseline reduite a `17` fichiers

12. fait: migrer `PipelineStageClosedWon.vue` hors `supabase.from('document')`

- fait: `front/src/components/opportunity/components/pipeline/components/PipelineStageClosedWon/PipelineStageClosedWon.vue` migre vers API backend `document`
- fait: selection explicite de la facture la plus recente cote frontend pour conserver le comportement precedent
- fait: tests unitaires frontend executes sur le client API utilise
- fait: guardrail supabase-direct valide, baseline reduite a `16` fichiers

13. fait: migrer `ActionsPage.vue` hors `supabase.from('action'|'action_execution_log')`

- fait: nouveau client frontend `front/src/api/action.ts`
- fait: `front/src/components/opportunity/components/actions/ActionsPage.vue` migre vers API backend `action`
...
16. fait: migrer `OpportunityHeader.vue` hors `supabase.from('opportunity'|'document')`

- fait: ajout endpoint backend `POST /api/opportunities/create-draft`
- fait: extensions client frontend `front/src/api/opportunity.ts` (`createDraftOpportunity`, `searchOpportunities`)
- fait: `front/src/components/opportunity/OpportunityHeader.vue` migre vers APIs backend `opportunity` et `document`
- fait: tests unitaires backend/frontend ajoutes pour la nouvelle route et les helpers associes
- fait: guardrail supabase-direct valide, baseline reduite a `12` fichiers

17. fait: sortir `IndexPage.vue` de l'allowlist `supabase-direct`

- fait: suppression du reliquat `import { supabase }` inutile dans `front/src/components/opportunity/IndexPage.vue`
- fait: guardrail supabase-direct valide, baseline reduite a `11` fichiers

18. fait: migrer `components/invoices/InvoicesPage.vue` hors `supabase.from('document')`

- fait: `front/src/components/opportunity/components/invoices/InvoicesPage.vue` migre vers API backend `document`
- fait: tests unitaires frontend executes sur le client API utilise
- fait: guardrail supabase-direct valide, baseline reduite a `10` fichiers

19. fait: migrer `components/invoices/InvoiceDetailPage.vue` hors `supabase.from('document'|'sent_email'|'opportunity'|'contact')`

- fait: `front/src/components/opportunity/components/invoices/InvoiceDetailPage.vue` migre vers APIs backend `document`, `opportunity` et `contact`
- fait: tests unitaires frontend executes sur les clients API reutilises
- fait: guardrail supabase-direct valide, baseline reduite a `9` fichiers

20. fait: migrer `front/src/components/invoices/InvoicesPage.vue` hors `supabase.from('document')`

- fait: `front/src/components/invoices/InvoicesPage.vue` migre vers API backend `document`
- fait: tests unitaires frontend executes sur le client API utilise
- fait: guardrail supabase-direct valide, baseline reduite a `8` fichiers

21. fait: migrer `front/src/components/invoices/InvoiceDetailPage.vue` hors `supabase.from('document'|'sent_email'|'opportunity'|'contact')`

- fait: `front/src/components/invoices/InvoiceDetailPage.vue` migre vers APIs backend `document`, `opportunity` et `contact`
- fait: tests unitaires frontend executes sur les clients API reutilises
- fait: guardrail supabase-direct valide, baseline reduite a `7` fichiers

22. fait: migrer `front/src/components/products/DetailPage.vue` hors `supabase.from('product')`

- fait: nouveau client frontend `front/src/api/product.ts` (`getProduct`)
- fait: `front/src/components/products/DetailPage.vue` migre vers API backend `product`
- fait: test unitaire frontend ajoute pour le helper `getProduct`
- fait: guardrail supabase-direct valide, baseline reduite a `6` fichiers

23. fait: migrer `front/src/components/contact/DetailPage.vue` hors `supabase.from('opportunity_participant')`

- fait: ajout endpoint backend `GET /api/contact/{id}/opportunities`
- fait: extension client frontend `front/src/api/contact.ts` (`listContactOpportunities`)
- fait: `front/src/components/contact/DetailPage.vue` migre vers API backend `contact`
- fait: tests unitaires backend/frontend ajoutes pour la nouvelle route et le helper associe
- fait: guardrail supabase-direct valide, baseline reduite a `5` fichiers

24. fait: migrer `front/src/components/products/IndexPage.vue` hors `supabase.from('product'|'family'|'product_family')`

- fait: ajout endpoint backend `GET /api/products/search`
- fait: extension client frontend `front/src/api/product.ts` (`searchProducts`)
- fait: `front/src/components/products/IndexPage.vue` migre vers API backend `product`
- fait: tests unitaires backend/frontend ajoutes pour la nouvelle route et le helper associe
- fait: guardrail supabase-direct valide, baseline reduite a `4` fichiers

25. fait: migrer `front/src/components/opportunity/components/quote/QuoteDocument.vue` hors `supabase.from('product'|'family')`

- fait: ajout endpoint backend `GET /api/products/quote-context`
- fait: extension client frontend `front/src/api/product.ts` (`getQuoteProductsContext`)
- fait: `front/src/components/opportunity/components/quote/QuoteDocument.vue` migre vers API backend `product`
- fait: tests unitaires backend/frontend ajoutes pour la nouvelle route et le helper associe
- fait: guardrail supabase-direct valide, baseline reduite a `3` fichiers

26. fait: migrer `front/src/components/mail/IndexPage.vue` hors `supabase.from('email_attachment')`

- fait: ajout endpoint backend `GET /api/email/{id}/attachments`
- fait: nouveau client frontend `front/src/api/email.ts` (`listEmailAttachments`)
- fait: `front/src/components/mail/IndexPage.vue` migre vers API backend email pour les pieces jointes
- fait: `supabase` reste uniquement pour `auth`/`realtime` dans cet ecran
- fait: tests unitaires backend/frontend ajoutes pour la nouvelle route et le helper associe

27. fait: resserrer `front/scripts/check-supabase-direct.mjs` sur les acces metier `supabase.from/rpc`

- fait: les usages `supabase.auth.*` et `supabase.channel(...)` sortent du comptage guardrail
...
- fait: ajout endpoint backend `GET /api/invoice?opportunity_id=...` (liste factures)
- fait: ajout endpoint backend `GET /api/invoice/{id}/view?opportunity_id=...` (invoice + sent_email + default_contact)
- fait: nouveau client frontend `front/src/api/invoice.ts` (`listOpportunityInvoices`, `getOpportunityInvoiceView`)
- fait: migration de `front/src/components/invoices/InvoicesPage.vue` et `front/src/components/opportunity/components/invoices/InvoicesPage.vue` vers `front/src/api/invoice.ts`
- fait: migration de `front/src/components/invoices/InvoiceDetailPage.vue` et `front/src/components/opportunity/components/invoices/InvoiceDetailPage.vue` vers `front/src/api/invoice.ts`
- fait: tests unitaires backend/frontend ajoutes pour les nouveaux endpoints/helpers
